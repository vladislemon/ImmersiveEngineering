...
public class TileEntityThermoelectricGen extends TileEntityIEBase implements IEnergyConnection {
    ...
    Fluid getFluid(int x, int y, int z) {
        ...
        if (f == null && b instanceof BlockDynamicLiquid && worldObj.getBlockMetadata(x, y, z) == 0) if (b.getMaterial()
            .equals(Material.water)) f = FluidRegistry.WATER;
        else if (b.getMaterial()
            .equals(Material.lava)) f = FluidRegistry.LAVA;
