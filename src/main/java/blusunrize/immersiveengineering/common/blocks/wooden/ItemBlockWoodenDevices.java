...
public class ItemBlockWoodenDevices extends ItemBlockIEBase {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (meta == 0) {
            for (int i = 0; i <= 3; i++)
                if (!player.canPlayerEdit(x, y + 1, z, side, stack) || !world.getBlock(x, y + i, z)
                    .isReplaceable(world, x, y + i, z)) return false;
        } else if (meta == 1) for (int yy = -2; yy <= 2; yy++) {
            int r = yy < -1 || yy > 1 ? 1 : 2;
            for (int ww = -r; ww <= r; ww++)
                if (!world.getBlock(x + ((f == 2 || f == 3) ? ww : 0), y + yy, z + ((f == 2 || f == 3) ? 0 : ww))
                    .isReplaceable(world, x + ((f == 2 || f == 3) ? ww : 0), y + yy, z + ((f == 2 || f == 3) ? 0 : ww)))
                    return false;
        }
        else if (meta == 2 || meta == 3) for (int yy = -6; yy <= 6; yy++) {
            int r = Math.abs(yy) == 6 ? 1 : Math.abs(yy) == 5 ? 3 : Math.abs(yy) == 4 ? 4 : Math.abs(yy) > 1 ? 5 : 6;
            for (int ww = -r; ww <= r; ww++) if (!world.getBlock(x + (f <= 3 ? ww : 0), y + yy, z + (f <= 3 ? 0 : ww))
        ...
        if (tileEntity instanceof TileEntityWoodenPost) {
            ...
            for (int yy = -2; yy <= 2; yy++) {
                int r = yy < -1 || yy > 1 ? 1 : 2;
                for (int ww = -r; ww <= r; ww++) if (yy != 0 || ww != 0) {
                    world.setBlock(
                        x + ((f == 2 || f == 3) ? ww : 0),
                        y + yy,
                        z + ((f == 2 || f == 3) ? 0 : ww),
                        ...
                        meta,
                        0x3);
                    tileEntityWatermill = world
                        .getTileEntity(x + ((f == 2 || f == 3) ? ww : 0), y + yy, z + ((f == 2 || f == 3) ? 0 : ww));
            ...
            if (!world.getBlock(x + xOff, y, z + zOff)
                .isReplaceable(world, x + xOff, y, z + zOff)) {
