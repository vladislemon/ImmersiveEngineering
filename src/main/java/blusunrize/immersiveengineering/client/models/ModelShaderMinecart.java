...
public class ModelShaderMinecart extends ModelMinecart {
    ...
    public ModelShaderMinecart(ModelMinecart model) {
        ...
        for (ModelBox cube : (List<ModelBox>) sideModelsMirrored[4].cubeList) newCubes.add(
            new ModelBox(
        ...
        if (sCase instanceof ShaderCaseMinecart) {
            ...
            for (int part = 0; part < sideModels.length - 1; part++) if (sideModels[part] != null) {
                ...
                for (int pass = 0; pass < maxPasses; pass++)
                    if ((((ShaderCaseMinecart) sCase).additionalTexture != null && pass == 2) ? (part != 1 && part != 2)
                        : (pass != 1 || ((ShaderCaseMinecart) sCase).overlaySides[part])) {
                            ...
                            GL11.glColor4f(
                                ...
                                col[1] / 255f,
                                col[2] / 255f,
                                col.length > 3 ? (col[3] / 255f) : 1f);

                            if (pass == maxPasses - 1) ClientUtils
                                .bindTexture("immersiveengineering:textures/models/shaders/minecart_uncoloured.png");
                            else if (pass == maxPasses - 2 && ((ShaderCaseMinecart) sCase).additionalTexture != null)
                                ClientUtils.bindTexture(
                                    sCase.getBaseTexturePath() + ((ShaderCaseMinecart) sCase).additionalTexture
                                        + ".png");
                            else if (pass == 0)
                                ClientUtils.bindTexture("immersiveengineering:textures/models/shaders/minecart_0.png");
                            else {
                                ClientUtils
                                    .bindTexture(sCase.getBaseTexturePath() + "1_" + sCase.getOverlayType() + ".png");
                                upScale = false;
                            }
                            ...
                            if (icon != null) {
                                ...
                                if (modelRemapped != null) if (((ShaderCaseMinecart) sCase).mirrorSideForPass[pass])
    ...
    ModelShaderMinecart getRemappedModel(IIcon icon) {
        ...
        modelRemapped.sideModelsMirrored = ClientUtils
            .copyModelRenderers(modelRemapped, modelRemapped.sideModelsMirrored);
